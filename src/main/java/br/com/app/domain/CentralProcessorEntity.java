...
public class CentralProcessorEntity {
   ...
   // private[] Double systemLoadAverage(int nelem);
    ...
    public CentralProcessorEntity() {}
    public  CentralProcessorEntity(CentralProcessor centralProcessor)
    {
    	this.vendor = centralProcessor.getVendor();
    	this.name = centralProcessor.getName();
    	this.vendorFreq = centralProcessor.getVendorFreq();
    	this.processorID = centralProcessor.getProcessorID();
    	this.identifier = centralProcessor.getIdentifier();
    	this.isCpu64bit = centralProcessor.isCpu64bit();
    	this.stePping = centralProcessor.getStepping();
    	this.model = centralProcessor.getModel();
    	this.family = centralProcessor.getFamily();
    	this.systemCpuLoadBetweenTicks = centralProcessor.getSystemCpuLoadBetweenTicks();
    	this.systemCpuLoadTicks = new ArrayList<Long>();
    	for(Long systemCpuLoadTick : centralProcessor.getSystemCpuLoadTicks())
    	{
    		systemCpuLoadTicks.add(systemCpuLoadTick);
    	}
    	this.systemCpuLoad = centralProcessor.getSystemCpuLoad();
    	this.systemLoadAverage = centralProcessor.getSystemLoadAverage();
    	this.processorCpuLoadBetweenTicks = new ArrayList<Double>();
    	for(Double processorCpuLoadBetweenTick : centralProcessor.getProcessorCpuLoadBetweenTicks()){
    		
    		processorCpuLoadBetweenTicks.add(processorCpuLoadBetweenTick);
    	}
    	this.systemUptime = centralProcessor.getSystemUptime();
